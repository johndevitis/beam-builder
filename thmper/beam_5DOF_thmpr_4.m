...
fprintf('Pole %d\tReal: %6.3f\tImag: %6.3f\n',...
...
Sk = trackShapes(L,ss,uu,peakLoc,w,lb,rb,thresh);
% plot
[~,bnds] = searchVector(w,[lb rb]);
ph = plot(ah,w(bnds(1):bnds(2)),mag2db(Sk),'.-');
set(ph,'linewidth',1,...
    'markersize',12);
lg = legend(ah,{'Mode 1'; 'Mode 2'; 'Mode 3'; 'Mode 4';'Mode 5'});
set(ah,'fontsize',18,...
...
% get poles
[eH,pols,frdmp,qr,ma,ehs] = cmif_getpoles(hh,U,Lm,dp,f,peakLoc,noterms,nob);
...
for ii = 1:nn
    ...
    plot(ah,f,mag2db(abs(eH(:,ii))),'.-')
    hold(ah,'all')
    plot(ah,f,mag2db(abs(ehs(:,ii))),'--')
    plot(ah,f(peakLoc(ii)),mag2db(abs(eH(peakLoc(ii),ii))),'ro');
    hold(ah,'off');
    xlim(ah,[0 150]);
